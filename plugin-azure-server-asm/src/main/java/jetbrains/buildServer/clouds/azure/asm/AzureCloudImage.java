...
public class AzureCloudImage extends AbstractCloudImage<AzureCloudInstance, AzureCloudImageDetails> {
  ...
  AzureCloudImage(@NotNull final AzureCloudImageDetails imageDetails,
    ...
    myGeneralized = !myImageDetails.getBehaviour().isUseOriginal();
  }
  ...
  public boolean canStartNewInstance() {
    if (myImageDetails.getBehaviour().isUseOriginal()) {
      final AzureCloudInstance singleInstance = CollectionsUtil.findFirst(getInstances(), new Filter<AzureCloudInstance>() {
        @Override
        public boolean accept(@NotNull AzureCloudInstance data) {
          return myImageDetails.getSourceId().equalsIgnoreCase(data.getInstanceId());
        }
      });
      return singleInstance != null && singleInstance.getStatus() == InstanceStatus.STOPPED;
    } else {
      return getInstances().size() < myImageDetails.getMaxInstances()
  ...
  public AzureCloudInstance startNewInstance(@NotNull final CloudInstanceUserData tag) {
    ...
    if (myImageDetails.getBehaviour().isUseOriginal()) {
      vmName = myImageDetails.getSourceId();
      instance = CollectionsUtil.findFirst(getInstances(), new Filter<AzureCloudInstance>() {
        @Override
        public boolean accept(@NotNull AzureCloudInstance data) {
          return myImageDetails.getSourceId().equalsIgnoreCase(data.getInstanceId());
        }
      ...
      if (getInstances().size() >= myImageDetails.getMaxInstances()) {
